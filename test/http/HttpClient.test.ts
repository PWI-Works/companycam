...
const axiosMocks = vi.hoisted(() => {
  ...
  const isAxiosErrorMock = vi.fn((error: unknown) => Boolean((error as { isAxiosError?: boolean })?.isAxiosError));
...
const { axiosRequest, axiosCreate, isAxiosErrorMock, createInstance } = axiosMocks;
...
describe('HttpClient', () => {
  ...
  it('attaches default headers and honours per-request overrides', async () => {
    ...
    axiosRequest.mockResolvedValue({ status: 200, data: {}, headers: {}, statusText: 'OK' } as unknown as AxiosResponse);
  ...
  it('wraps Axios errors in APIError instances', async () => {
    ...
    const client = new HttpClient({
      rateLimiter: { acquire: vi.fn(() => Promise.resolve()), dispose: vi.fn() } as unknown as RateLimiter,
    });
    ...
    const failingRequest = client.request({ method: 'get', url: '/failing-endpoint' }).catch((err) => {
    ...
    const thrownError = capturedError as APIError;

    expect(thrownError).toBeInstanceOf(APIError);
    expect(thrownError.status).toBe(500);
    expect(thrownError.code).toBe('server_error');
    expect(thrownError.requestId).toBe('req-789');
    expect(thrownError.method).toBe('GET');
    expect(thrownError.url).toBe('/failing-endpoint');
  });
  ...
  it('disposes owned rate limiters while leaving externally supplied ones alone', () => {
    ...
    const externalLimiter = { acquire: vi.fn(), dispose: vi.fn() } as unknown as RateLimiter;
  ...
  it('evaluates retry eligibility according to method and status', () => {
    ...
    isNetworkErrorMock.mockImplementation((error: unknown) => error === networkError);

    // Act & Assert: GET retries on server errors, PATCH retries on 429, POST respects config, network errors retry.
    expect((defaultClient as unknown as { shouldRetry(error: AxiosError): boolean }).shouldRetry(serverError)).toBe(
      true
    );
    expect((defaultClient as unknown as { shouldRetry(error: AxiosError): boolean }).shouldRetry(postError)).toBe(
      false
    );
    expect((postRetryClient as unknown as { shouldRetry(error: AxiosError): boolean }).shouldRetry(postError)).toBe(
      true
    );
    expect((defaultClient as unknown as { shouldRetry(error: AxiosError): boolean }).shouldRetry(rateLimited)).toBe(
      true
    );
    expect((defaultClient as unknown as { shouldRetry(error: AxiosError): boolean }).shouldRetry(networkError)).toBe(
      true
    );
  });
  ...
  it('honours Retry-After headers when computing backoff delays', () => {
    ...
    const retryAfterFutureDate = {
      response: { headers: { 'retry-after': new Date(Date.now() + 3000).toUTCString() } },
    } as AxiosError;

    const noHeader = {} as AxiosError;

    // Act: evaluate different delay scenarios.
    const delayFromSeconds = (client as unknown as { computeRetryDelay(retryCount: number, error: AxiosError): number })
      .computeRetryDelay(1, retryAfterSeconds);
    const delayFromDate = (client as unknown as { computeRetryDelay(retryCount: number, error: AxiosError): number })
      .computeRetryDelay(1, retryAfterFutureDate);
    const delayWithoutHeader = (
      client as unknown as { computeRetryDelay(retryCount: number, error: AxiosError): number }
