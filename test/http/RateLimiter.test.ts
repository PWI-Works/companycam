...
describe('RateLimiter', () => {
  ...
  it('respects abort signals supplied before enqueueing', async () => {
    ...
    await expect(limiter.acquire(controller.signal as unknown as AbortSignal)).rejects.toMatchObject({
      name: 'AbortError',
  ...
  it('removes queued requests when their abort signal fires', async () => {
    ...
    const controller = new AbortController();
    const queuedAttempt = limiter.acquire(controller.signal as unknown as AbortSignal);
